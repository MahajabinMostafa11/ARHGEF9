...
<!--
  ...
  instantiation time:
    ARHGEF9   Human-readable project name.
    ARHGEF9      Repository slug (used for the wiki path).
...
The wiki at `wiki/ARHGEF9.wiki/` is a separate git repo with its own history and its own remote. After any wiki edit, commit in the wiki repo (not the project repo):

```bash
git -C wiki/ARHGEF9.wiki add <files>
git -C wiki/ARHGEF9.wiki commit -m "..."
```

Push the wiki only when you intend to publish the changes:

```bash
git -C wiki/ARHGEF9.wiki push origin master
```
